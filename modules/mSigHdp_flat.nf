process mSigHdp_flat {
    ...
    script:
    """
    Rscript --vanilla ${projectDir}/bin/mSigHdp_flat.R -c ${mutational_context} -a ${analysis_type} -b ${burnin_iterations} -x ${burnin_multiplier} -o ${posterior} -i ${posterior_iterations} -cp ${concentration_parameter} -ch ${chains} -k ${clusters} -ga ${alpha} -gb ${beta} -cf ${confidence} ${mutational_matrix}
    """
}